// -*- C++ -*-
//
// Package:    DelauIdeal/Moge_tightID
// Class:      Moge_tightID
// 
/**\class Moge_tightID Moge_tightID.cc DelauIdeal/Moge_tightID/plugins/Moge_tightID.cc
...
class Moge_tightID : public edm::EDAnalyzer {
   public:
      explicit Moge_tightID(const edm::ParameterSet&);
      ~Moge_tightID();
...
//
Moge_tightID::Moge_tightID(const edm::ParameterSet& iConfig):
...
void
Moge_tightID::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  clearTree();
...
// https://twiki.cern.ch/twiki/bin/view/CMSPublic/SWGuideMuonId and read for Run2
  ...
  for (const pat::Muon &mu : *muons) {
    ...
    muChrg.push_back(mu.charge());
    if( !(mu.pt() > 10 && abs(mu.eta()) < 2.4)) continue;
    if( !mu.innerTrack().isNonnull() ) continue;
    if( !mu.globalTrack().isNonnull() ) continue;
    if( !(mu.isTightMuon(PV_na))) continue;
  ...
  for (unsigned j = 0; j < GoodRecoMu.size(); j++) {
      ...
      muPt_Good.push_back(GoodRecoMu[j].p4.Pt());
      muEta_Good.push_back(GoodRecoMu[j].p4.Eta());
      muPhi_Good.push_back(GoodRecoMu[j].p4.Phi());
      muChrg_Good.push_back(GoodRecoMu[j].Scharge);
      bool HLT_mu = RecoHLTMuonMatching(GoodRecoMu[j].p4.Eta(),GoodRecoMu[j].p4.Phi());
  ...
  if (GoodRecoMu.size() > 1){
   ...
   for(unsigned led = 0; led < GoodRecoMu.size(); led++) {
      if (!(GoodRecoMu[led].p4.Pt() > 22)) continue;

      for(unsigned sub = 0; sub < GoodRecoMu.size(); sub++) {
         if (led == sub) continue;
         // Angle
         TVector3 Mu1(GoodRecoMu[led].p4.Px(),GoodRecoMu[led].p4.Py(), GoodRecoMu[led].p4.Pz());
         TVector3 Mu2(GoodRecoMu[sub].p4.Px(), GoodRecoMu[sub].p4.Py(), GoodRecoMu[sub].p4.Pz());
         double cos_angle = Mu1.Dot(Mu2) / GoodRecoMu[led].p4.P() / GoodRecoMu[sub].p4.P();
         Angle_Good = cos_angle;
         //trigger
         bool HLT_mu1 = RecoHLTMuonMatching(GoodRecoMu[led].p4.Eta(),GoodRecoMu[led].p4.Phi());
         bool HLT_mu2 = RecoHLTMuonMatching(GoodRecoMu[sub].p4.Eta(),GoodRecoMu[sub].p4.Phi());
         //vertex
         ...
         if (!(GoodRecoMu[sub].p4.Pt() > 10)) continue;
   ...
   muPt_Sel.push_back(GoodRecoMu[gdInx_mu1].p4.Pt());
   muPt_Sel.push_back(GoodRecoMu[gdInx_mu2].p4.Pt());
   DYPt_hist     -> Fill(GoodRecoMu[gdInx_mu1].p4.Pt());
   DYPt_hist     -> Fill(GoodRecoMu[gdInx_mu2].p4.Pt());
   DYPt_lead     -> Fill(GoodRecoMu[gdInx_mu1].p4.Pt());
   DYPt_sub      -> Fill(GoodRecoMu[gdInx_mu2].p4.Pt());

   muEta_Sel.push_back(GoodRecoMu[gdInx_mu1].p4.Eta());
   muEta_Sel.push_back(GoodRecoMu[gdInx_mu2].p4.Eta());
   DYEta_hist    -> Fill(GoodRecoMu[gdInx_mu1].p4.Eta());
   DYEta_hist    -> Fill(GoodRecoMu[gdInx_mu2].p4.Eta());
   DYEta_lead    -> Fill(GoodRecoMu[gdInx_mu1].p4.Eta());
   DYEta_sub     -> Fill(GoodRecoMu[gdInx_mu2].p4.Eta());

   muPhi_Sel.push_back(GoodRecoMu[gdInx_mu1].p4.Phi());
   muPhi_Sel.push_back(GoodRecoMu[gdInx_mu2].p4.Phi());
   DYPhi_hist    -> Fill(GoodRecoMu[gdInx_mu1].p4.Phi());
   DYPhi_hist    -> Fill(GoodRecoMu[gdInx_mu2].p4.Phi());
   DYPhi_lead    -> Fill(GoodRecoMu[gdInx_mu1].p4.Phi());
   DYPhi_sub     -> Fill(GoodRecoMu[gdInx_mu2].p4.Phi());

   muChrg_Sel.push_back(GoodRecoMu[gdInx_mu1].Scharge);
   muChrg_Sel.push_back(GoodRecoMu[gdInx_mu2].Scharge);

   TLorentzVector mu1, mu2;
   mu1.SetPtEtaPhiM(GoodRecoMu[gdInx_mu1].p4.Pt(),GoodRecoMu[gdInx_mu1].p4.Eta(),GoodRecoMu[gdInx_mu1].p4.Phi(), 0.1056583);
   mu2.SetPtEtaPhiM(GoodRecoMu[gdInx_mu2].p4.Pt(),GoodRecoMu[gdInx_mu2].p4.Eta(),GoodRecoMu[gdInx_mu2].p4.Phi(), 0.1056583);
   mmM_Sel = (mu1+mu2).M();
...
void 
Moge_tightID::beginRun(edm::Run const&, edm::EventSetup const&)
{
}
...
void 
Moge_tightID::endRun(edm::Run const&, edm::EventSetup const&)
{
}
...
void 
Moge_tightID::beginLuminosityBlock(edm::LuminosityBlock const&, edm::EventSetup const&)
{
}
...
void 
Moge_tightID::endLuminosityBlock(edm::LuminosityBlock const&, edm::EventSetup const&)
{
}
...
void
Moge_tightID::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
DEFINE_FWK_MODULE(Moge_tightID);

bool Moge_tightID::isPassHLT(){ 
  int nbMatch = 0;
...
bool Moge_tightID::RecoHLTMuonMatching(float RecoEta,float RecoPhi){
  int nbMatch  = 0;
...
float Moge_tightID::delR(float eta1,float phi1,float eta2,float phi2){
	float mpi=3.14;
